...
public class FrameService {
    ...
    public FrameService() { easingServiceFactory = new EasingServiceFactory(); }

    public List<Frame> mergeFrames(List<Frame> frames, List<Frame> input) {
    ...
    public List<Frame> convertStepsIntoFrames(List<TransitionStep> transitionSteps) {
        ...
        IntStream.range(0, transitionSteps.size())
                .forEach(i -> frames.addAll(
                        createFrames(
    ...
    List<Frame> createFrames(TransitionStep current, TransitionStep next) {
        ...
        // Add freeze frame if requested
        if (current.getSleep() > 0) {
            frames.add(new Frame(current.getLeds(), current.getSleep()));
        }
        ...
        if (current.getTransitionTime() > 0) {
            frames.addAll(createTransitionFrames(
                    ...
                    EasingType.LINEAR) // EasingType hardcoded for now.
                                       // In the future it should be possible to inherit it from the request
            );
        }
    ...
    List<Frame> mergeLedTransitions(List<Frame> frames, List<Color> input) {
        IntStream.range(0, input.size())
                .forEach(i -> frames.get(i).getLeds().add(input.get(i)));
        return frames;
    }
}
